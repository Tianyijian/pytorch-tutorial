...
class Ai18Processor(DataProcessor):
    def get_train_examples(self, data_dir, aspect):
        return self._create_examples(data_dir + "/sentiment_analysis_trainingset.csv", "train" , aspect)

    def get_dev_examples(self, data_dir, aspect):
        return self._create_examples(data_dir + "/sentiment_analysis_validationset.csv", "dev", aspect)

    def get_labels(self):
        return ["-2", "-1", "0", "1"]

    def _create_examples(self, data_dir, set_type, aspect):
        import csv
        with open(data_dir, "r", encoding="utf-8") as f:
            ...
            for i, line in enumerate(reader):
                if i == 0:
                    continue
                guid = "%s-%s-%s" % (set_type, aspect, line[0])
                text_a = line[1]
                label = line[aspect + 2]
                examples.append(InputExample(
